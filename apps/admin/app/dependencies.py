...
# 보안: 기본값 제거. .env.local 또는 Railway Variables에 반드시 설정 필요
_api_key_raw = os.environ.get("NEXT_PUBLIC_X_API_KEY", "").strip()
if not _api_key_raw:
    raise ValueError(
        "NEXT_PUBLIC_X_API_KEY 환경 변수가 설정되지 않았습니다. "
        ".env.local 또는 Railway Variables에 NEXT_PUBLIC_X_API_KEY를 추가하세요."
    )
API_SECRET_KEY = _api_key_raw

...
• Google Gemini API (Google LLC, 미국): 서버에서 집계된 텍스트·수치(뉴스 제목·요약, 수급 수치 등)를 입력으로 하여 AI 요약·시황 문장·이슈 정리, 영문 리포트 한글 번역, 목표가 조정 기사 정보 추출, 시장 논조 요약 등을 수행합니다. 본 처리에는 이용자를 직접 식별하는 정보를 넣지 않습니다.

(참고) Gemini 자동 처리 시각 한 줄 요약 — KST, 거래일·스케줄·공휴일 설정에 따름
• 수급 AI 요약 — 평일 08:30~20:30 약 30분 간격(수급 수집 직후)
• 금일 이슈 AI 요약 — 평일 08:30 / 12:30 / 19:30(주가영향 뉴스 수집과 함께)
• 시장의 목소리 AI 요약 — (자동 스케줄 중단: 토큰 절감) 과거에는 평일 08:30 / 12:30 / 19:30 뉴스 파이프라인과 함께 실행
• 장마감 시황 게시 초안 — 평일 16:25
• 투자은행 뉴스 번역 — (자동 스케줄 중단: 토큰 절감) 과거에는 매일 12:00 KST
• 목표가 조정 뉴스 추출 — 평일 08:30 / 12:00
• 운영자 수동 실행 — 관리자 화면·API로 장마감 시황·금일 이슈·시장의 목소리 등 동일 AI 기능 트리거 가능
...
4. "AI 추천"이란 Google Gemini API를 통한 서버 측 자동 요약·분석 결과, 또는 자체 알고리즘(일목균형표·종가베팅 스크리닝 등)이 생성한 결과물을 의미합니다.
...
• 뉴스·시황: 네이버 뉴스, Google Gemini 기반 AI 요약·이슈 정리, 증권사 리포트, 목표가 뉴스 (비회원 포함)
• 수급 정보: 기관/외국인 수급 현황, Google Gemini 기반 AI 수급 요약 (비회원 포함)

...
type FortuneType = "zodiac" | "animal" | "mbti";
...
const MBTI_INVEST_STYLE: Record<string, string> = {
  ...
  ISTP: "단기 기회형", ESTP: "고수익 트레이더", ENFJ: "테마 섹터형", ENFP: "트렌드 포착형",
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "var(--app-bg)", padding: "0 0 6rem" }}>
      ...
      <div style={{
        ...
        <span>
          KOSPI <span style={{ color: "var(--app-up)", fontFamily: "monospace" }}>▲ 2,620.15 +1.23%</span>
        </span>
        <span>
          KOSDAQ <span style={{ color: "var(--app-up)", fontFamily: "monospace" }}>▲ 781.40 +0.87%</span>
        </span>
      </div>

      <div style={{ padding: "1rem" }}>
        {/* 날짜 */}
        <p style={{ color: "var(--app-text-muted)", fontSize: "0.85rem", marginBottom: "1rem" }}>
          {TODAY}
        </p>
        ...
        {activeTab === "animal" && (
          <FortuneCard
            ...
            fortune={animal ? `${animal}띠의 오늘은 신중한 관망이 유리합니다. 충동적인 매수보다 분석 후 진입하세요.` : undefined}
            tip={animal ? "분할 매수 전략이 유효한 하루입니다." : undefined}
            stars={animal ? 4 : 0}
          />
        )}
        ...
        {activeTab === "zodiac" && (
          <FortuneCard
            ...
            fortune={sign ? `${sign}의 오늘은 기회를 포착하기 좋은 날입니다. 준비된 종목에 과감히 진입해 보세요.` : undefined}
            tip={sign ? "손절 라인을 미리 정해두고 진입하는 것이 좋습니다." : undefined}
            stars={sign ? 3 : 0}
          />
        )}
...
function FortuneCard({
  hasValue, emoji, title, noValueContent, fortune, tip, stars,
}: {
  hasValue: boolean;
  emoji: string;
  title: string;
  ...
  return (
    <div style={{
      ...
      {hasValue && fortune ? (
        <>
          <p style={{ color: "var(--app-text-secondary)", lineHeight: 1.7, marginBottom: "1rem" }}>{fortune}</p>
          {tip && (
            <div style={{
